...
def resnet_v2(inParams, data_format=None):
  ...
  if inParams['resnet_size']  not in model_params:
    raise ValueError('Not a valid resnet_size:', inParams['resnet_size'])

  params = model_params[inParams['resnet_size']]
...
def resnetv2_model_fn(features, labels, mode, params):
  ...
  pred_gender = tf.argmax(input=logits_gender, axis=1, output_type=tf.int32)

  # Age regression
  num_classes_age = 1
  final_dense_age = tf.compat.v1.layers.dense(inputs=final, units=num_classes_age)
  ...
  # Regression loss
  loss_mse = tf.compat.v1.losses.mean_squared_error(labels['age'], pred_age)
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    train_op = optimizer.minimize(pred_loss, global_step=tf.compat.v1.train.get_global_step())
